...
# 주식주봉차트조회요청
def fn_ka10082(token, data, host=None, cont_yn='N', next_key=''):
	# 1. 요청할 API URL
	if host is None:
...
# 키움증권 API 주봉 데이터를 표준 OHLCV 형식으로 전처리
def preprocess_kiwoom_weekly_data(df):
	"""
	키움증권 API에서 가져온 주봉 데이터를 표준 OHLCV 형식으로 변환합니다.
...
def save_to_csv(data, filename, code=None, preprocess=True):
	...
	# 전처리 수행
	if preprocess:
		processed_df = preprocess_kiwoom_weekly_data(df)
...
# 특정 기간의 주봉 데이터 조회 및 저장
def get_weekly_chart(stock_code, period=None, end_date=None, modified_price_type='1'):
	"""
	특정 기간의 주봉 데이터를 조회하고 저장하는 함수
	
	Args:
		stock_code (str): 종목코드 (예: '005930')
		period (str, optional): 조회 기간 (예: '2y', '12m', '52w')
			- 'Xy': X년
			- 'Xm': X개월
			- 'Xw': X주
		end_date (str, optional): 조회 종료일(YYYYMMDD). 기본값은 오늘.
		modified_price_type (str): 수정주가구분 (0: 원주가, 1: 수정주가). 기본값은 '1'.
	
	Returns:
		DataFrame: 조회된 주봉 데이터
	"""
	...
	# 시작일 계산 (주봉은 일반적으로 4년 데이터를 요청)
	if period:
		start_date = calculate_start_date(end_date, period, trading_days=False)
	else:
		# 기본 기간: 4년 (208주)
		start_date = calculate_start_date(end_date, '4y', trading_days=False)
	...
	print(f"{stock_code} 종목의 {start_date}부터 {end_date}까지의 주봉 데이터를 조회합니다.")
	print(f"조회 기간: {period if period else '4y'}")
	...
	try:
		# 초기 요청
		result = fn_ka10082(token=token, data=params, host=config.host)
		...
		# 데이터 키 찾기
		data_key = None
		possible_keys = ['output2', 'stk_stk_pole_chart_qry', 'chart', 'list']
		...
		page_count += 1
		print(f"페이지 {page_count} 수신: {len(processed_data)}개 주봉 데이터 (전체: {len(all_data)}개)")
		...
		while (result['cont_yn'] == 'Y' and    # 연속 데이터가 있고
			...
			try:
				...
				if data_key in result and isinstance(result[data_key], list):
					...
					page_count += 1
					print(f"페이지 {page_count} 수신: {len(page_processed_data)}개 주봉 데이터 (전체: {len(all_data)}개)")
		...
		if all_data:
			# 날짜 필드 확인 및 처리
			if date_field in all_data[0]:
				# 데이터 정렬 (날짜 내림차순)
				all_data.sort(key=lambda x: x[date_field], reverse=True)
				
				# 실제 조회된 데이터의 날짜 범위
				latest_date = all_data[0][date_field]
				earliest_date = all_data[-1][date_field]
			...
			# 파일명: weekly_{stock_code}_{earliest_date}_{latest_date}.csv
			filename = os.path.join(output_dir, f'weekly_{stock_code}_{earliest_date}_{latest_date}.csv')
			
			# CSV 저장
			saved_file, df = save_to_csv(all_data, filename, code=stock_code, preprocess=True)
			
			if saved_file:
				print(f"\n{stock_code} 종목의 주봉 데이터가 저장되었습니다:")
				print(f"  - 전처리 데이터: {saved_file}")
				print(f"  - 원본 데이터: {saved_file.replace('.csv', '_original.csv')}")
				
				# 데이터 반환
				return df
			else:
				print(f"{stock_code} 종목의 주봉 데이터 저장에 실패했습니다.")
		else:
			print(f"{stock_code} 종목의 주봉 데이터가 없습니다.")
...
if __name__ == '__main__':
	...
	period = '2y'     # 기본값: 1년 데이터
	
	# 수정주가 적용 여부
	modified_price_type = '1'  # 1: 수정주가, 0: 원주가
	
	# 주봉 데이터 조회 및 저장
	weekly_data = get_weekly_chart(stock_code, period, modified_price_type=modified_price_type)
	...
	if weekly_data is not None and not weekly_data.empty:
		...
		print(f"행 수: {len(weekly_data)}")
		print(f"컬럼: {', '.join(weekly_data.columns)}")
