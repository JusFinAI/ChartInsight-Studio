...
# PatternManager 클래스 내
    ...
    def update_all(self, candle: Dict, newly_registered_peak: Optional[Dict], newly_registered_valley: Optional[Dict]):
        ...
        # Helper: convert datetime-like to (epoch_seconds, isostring)
        def _to_epoch_iso(dt_obj):
            try:
                if dt_obj is None:
                    return (None, None)
                ts = pd.to_datetime(dt_obj)
        ...
        for detector in self.active_detectors[:]: # Note the [:] for safe removal while iterating
            ...
            if detector.is_complete():
                ...
                if isinstance(detector, DoubleTopDetector):
                    # normalize start/end timestamps for frontend-friendly contract
                    start_dt = None
                    # 우선 detector.first_extremum.actual_date 우선 사용
                    if detector.first_extremum and detector.first_extremum.get('actual_date') is not None:
                        start_dt = detector.first_extremum.get('actual_date')
                    # fallback: first_extremum.detected_date
                    elif detector.first_extremum and detector.first_extremum.get('detected_date') is not None:
                        start_dt = detector.first_extremum.get('detected_date')
                    # fallback: try to infer from earliest structural point in detector attributes if present
                    try:
                        if start_dt is None and hasattr(detector, 'first_extremum') and isinstance(detector.first_extremum, dict):
                            for k in ['actual_date', 'detected_date']:
                                if detector.first_extremum.get(k) is not None:
                                    start_dt = detector.first_extremum.get(k); break
                    except Exception:
                        start_dt = start_dt

                    end_dt = candle.get('date') if candle.get('date') is not None else None
                    ...
                    start_epoch, start_iso = _to_epoch_iso(start_dt)
                    end_epoch, end_iso = _to_epoch_iso(end_dt)

                    # 개발 단계: start 시간이 없으면 에러 발생시켜 근본 원인 추적
                    if start_epoch is None:
                        logger.error(f"PatternManager: Missing start time for DoubleTopDetector. first_extremum={detector.first_extremum} end_dt={end_dt}")
                        raise RuntimeError(f"Missing start time for DT pattern (first_extremum={detector.first_extremum}, end={end_dt})")

                    self.completed_dt.append({
                        'date': candle['date'],
                        'price': candle['Close'],
                        'start_peak': detector.first_extremum,
                        'pattern_type': 'DT',
                    ...
                    if detector.first_extremum and detector.first_extremum.get('actual_date') is not None:
                        ...
                        start_dt = detector.first_extremum.get('detected_date')
                    try:
                        if start_dt is None and isinstance(detector.first_extremum, dict):
                            for k in ['actual_date', 'detected_date']:
                                if detector.first_extremum.get(k) is not None:
                                    start_dt = detector.first_extremum.get(k); break
                    except Exception:
                    ...
                    # 개발 단계: start 시간이 없으면 에러 발생
                    if start_epoch is None:
                        logger.error(f"PatternManager: Missing start time for DoubleBottomDetector. first_extremum={detector.first_extremum} end_dt={end_dt}")
                        raise RuntimeError(f"Missing start time for DB pattern (first_extremum={detector.first_extremum}, end={end_dt})")

                    self.completed_db.append({
                        'date': candle['date'],
                        'price': candle['Close'],
                        'start_valley': detector.first_extremum,
                        'pattern_type': 'DB',
                        'startTimeEpoch': start_epoch,
                        'endTimeEpoch': end_epoch,
                        'startTimeISO': start_iso,
                    ...
                    # derive start/end times (P1.actual_date as start if available)
                    start_dt = None
                    if detector.P1 and detector.P1.get('actual_date') is not None:
                        start_dt = detector.P1.get('actual_date')
                    elif detector.P1 and detector.P1.get('detected_date') is not None:
                        start_dt = detector.P1.get('detected_date')
                    try:
                        if start_dt is None and isinstance(detector.P1, dict):
                            for k in ['actual_date', 'detected_date']:
                                if detector.P1.get(k) is not None:
                                    start_dt = detector.P1.get(k); break
                    except Exception:
                        start_dt = start_dt
                    ...
                    # 개발 단계: start 시간이 없으면 에러 발생
                    if start_epoch is None:
                        logger.error(f"PatternManager: Missing start time for HeadAndShouldersDetector. P1={detector.P1} end_dt={end_dt}")
                        raise RuntimeError(f"Missing start time for HS pattern (P1={detector.P1}, end={end_dt})")
                    ...
                    # derive start/end times (V1.actual_date as start if available)
                    start_dt = None
                    if detector.V1 and detector.V1.get('actual_date') is not None:
                        start_dt = detector.V1.get('actual_date')
                    elif detector.V1 and detector.V1.get('detected_date') is not None:
                        start_dt = detector.V1.get('detected_date')
                    try:
                        if start_dt is None and isinstance(detector.V1, dict):
                            for k in ['actual_date', 'detected_date']:
                                if detector.V1.get(k) is not None:
                                    start_dt = detector.V1.get(k); break
                    except Exception:
                        start_dt = start_dt
                    ...
                    # 개발 단계: start 시간이 없으면 에러 발생
                    if start_epoch is None:
                        logger.error(f"PatternManager: Missing start time for InverseHeadAndShouldersDetector. V1={detector.V1} end_dt={end_dt}")
                        raise RuntimeError(f"Missing start time for IHS pattern (V1={detector.V1}, end={end_dt})")
...
# Kiwoom-specific loader and download helper removed for DB-only build.
# The DB-backed analyzer reads candles from the database and does not use
# the KiwoomDataLoader in this codepath. Keeping these functions here
# caused unnecessary imports/side-effects (kiwoom_api package initialization).
# If Kiwoom download functionality is required in the future, reintroduce
# a lazy import or provide a separate module for Kiwoom-only utilities.
