...
class ChartService:
    ...
    def get_minute_chart(self, stock_code, base_date=None, time_interval='1', 
        ...
        # API ID 설정 (정확한 분봉 API ID 사용)
        api_id = 'ka10080'  # 키움증권 공식 분봉 차트 API
        ...
        if result.data:
            print(f"키움 API 분봉 응답 데이터 개수: {len(result.data)}")
            if len(result.data) > 0:
                print(f"첫 번째 응답 아이템 키들: {list(result.data[0].keys())}")
                print(f"첫 번째 응답 아이템 샘플: {result.data[0]}")
            
            for item in result.data:
                # cntr_tm 필드 처리 (reference code 방식)
                if 'cntr_tm' in item:
                    timestamp = item['cntr_tm']
                    print(f"cntr_tm 원본 값: {timestamp}")
                    if len(timestamp) == 14:  # YYYYMMDDHHMMSS 형식인 경우
                        item['date'] = timestamp[:8]      # YYYYMMDD
                        item['time'] = timestamp[8:14]    # HHMMSS
                        print(f"분리된 date: {item['date']}, time: {item['time']}")
                    else:
