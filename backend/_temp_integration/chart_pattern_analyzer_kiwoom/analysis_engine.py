import numpy as np
import pandas as pd
# import yfinance as yf  # 키움증권 API로 교체
...
# logs 디렉토리가 없으면 모듈 폴더 기준으로 생성 (프로세스 CWD에 의존하지 않도록 수정)
MODULE_DIR = os.path.dirname(os.path.abspath(__file__))
MODULE_LOGS_DIR = os.path.join(MODULE_DIR, 'logs')
os.makedirs(MODULE_LOGS_DIR, exist_ok=True)

# 현재 날짜를 기반으로 로그 파일 이름 생성
current_date = datetime.now().strftime('%Y%m%d%H%M')
log_filename = os.path.join(MODULE_LOGS_DIR, f'dttb_strategy_grok_{current_date}.log')
...
# 글로벌 KiwoomDataLoader 인스턴스 (싱글톤 패턴)
_kiwoom_loader_instance = None

def get_kiwoom_loader():
    """KiwoomDataLoader 싱글톤 인스턴스 반환"""
    global _kiwoom_loader_instance
    if _kiwoom_loader_instance is None:
        _kiwoom_loader_instance = KiwoomDataLoader()
        logger.info("🔄 KiwoomDataLoader 싱글톤 인스턴스 생성")
    return _kiwoom_loader_instance

def download_data(ticker, period=None, interval='1d', retries=3, delay=15):
    """
    키움증권 API를 통한 데이터 다운로드 (V02 - 개선된 버전)
    
    Args:
        ticker (str): 6자리 종목 코드 (예: '005930')
        period (str): 사용되지 않음 (키움증권에서 최대 데이터 수집)
        interval (str): 타임프레임 ('5m', '30m', '1h', '1d', '1wk')
        retries (int): 재시도 횟수
        delay (int): 재시도 간격
        
    Returns:
        pd.DataFrame: OHLCV 데이터프레임 (Date 인덱스)
    """
    import logging
    logger = logging.getLogger(__name__)
    logger.info(f"[CRITICAL] download_data 함수 호출됨! ticker={ticker}, interval={interval}")
    logger.info(f"[CRITICAL] period={period}, retries={retries}, delay={delay}")
    logger.info(f"[CRITICAL] 파일 위치: {__file__}")
    logger.info(f"[CRITICAL] 함수명: download_data")
    
    # 키움 API 응답 원본 데이터 디버깅
    logger.info("=" * 50)
    logger.info("[CRITICAL] 키움 API 데이터 다운로드 시작")
    logger.info("=" * 50)
    
    # 캔들 개수 설정 (올바른 API 정보로 분봉 재활성화)
    count_map = {
        '5m': 500,    # 약 2일치
        '30m': 300,   # 약 1주치
        '1h': 200,    # 약 1주치
    ...
    count = count_map.get(interval, 500)
    
    logger.info(f"📊 데이터 요청: {ticker} {interval} {count}개 캔들")
    logger.info(f"[CRITICAL] 데이터 요청: {ticker} {interval} {count}개 캔들")
    
    for attempt in range(retries):
        logger.info(f"[CRITICAL] 시도 #{attempt + 1}/{retries}")
        try:
            # 싱글톤 키움증권 데이터 로더 사용
            logger.info(f"[CRITICAL] 키움 로더 가져오기...")
            loader = get_kiwoom_loader()
            logger.info(f"[CRITICAL] 키움 로더 가져오기 성공: {type(loader)}")
            
            # 데이터 다운로드
            logger.info(f"[CRITICAL] loader.download_data 호출 시작...")
            raw_data = loader.download_data(ticker, interval, count)
            logger.info(f"[CRITICAL] loader.download_data 호출 완료!")
            
            # [CRITICAL] 키움 API 응답 원본 데이터 디버깅
            logger.info(f"[CRITICAL] 키움 API 응답 확인:")
            logger.info(f"[CRITICAL] raw_data 타입: {type(raw_data)}")
            logger.info(f"[CRITICAL] raw_data.shape: {raw_data.shape if raw_data is not None else 'None'}")
            logger.info(f"[CRITICAL] raw_data 인덱스 타입: {type(raw_data.index) if raw_data is not None else 'None'}")
            if raw_data is not None and not raw_data.empty:
                logger.info(f"[CRITICAL] raw_data 컬럼: {list(raw_data.columns)}")
                logger.info(f"[CRITICAL] raw_data 첫 3행:")
                logger.info(f"\n{raw_data.head(3)}")
                logger.info(f"[CRITICAL] raw_data 인덱스 샘플: {raw_data.index[:5].tolist()}")
                
                # Date 컬럼의 실제 값들을 확인
                if 'Date' in raw_data.columns:
                    logger.info(f"[CRITICAL] Date 컬럼 실제 값들:")
                    logger.info(f"[CRITICAL] Date 컬럼 첫 5개: {raw_data['Date'].head().tolist()}")
                    logger.info(f"[CRITICAL] Date 컬럼 타입: {type(raw_data['Date'].iloc[0])}")
                    logger.info(f"[CRITICAL] Date 컬럼 샘플값: {repr(raw_data['Date'].iloc[0])}")
            
            if raw_data is None or raw_data.empty:
                raise ValueError("No data returned from Kiwoom API")
            
            # 키움 API는 이미 표준 컬럼명(Close, Open, High, Low, Volume, Date)을 사용
            # 필요한 컬럼만 선택 (없는 컬럼은 무시)
            ...
            logger.info(f"🔍 raw_data 컬럼: {list(raw_data.columns)}")
            logger.info(f"🔍 사용 가능한 표준 컬럼: {available_columns}")
            
            if available_columns:
                # 표준 컬럼들만 선택
                data = raw_data[available_columns].copy()
                
                # Date 컬럼이 있으면 변환 후 인덱스로 설정
                if 'Date' in data.columns:
                    logger.info(f"[CRITICAL] Date 컬럼 변환 시작:")
                    logger.info(f"[CRITICAL] 원본 Date 컬럼 타입: {type(data['Date'].iloc[0])}")
                    logger.info(f"[CRITICAL] Date 컬럼 샘플: {data['Date'].head().tolist()}")
                    logger.info(f"[CRITICAL] Date 컬럼 원본값 repr: {[repr(x) for x in data['Date'].head().tolist()]}")
                    
                    try:
                        logger.info(f"[CRITICAL] %Y%m%d 형식으로 변환 시도...")
                        data['Date'] = pd.to_datetime(data['Date'], format='%Y%m%d')
                        logger.info(f"[CRITICAL] Date 변환 성공! 변환된 샘플: {data['Date'].head().tolist()}")
                    except Exception as date_err:
                        logger.info(f"[CRITICAL] Date 변환 실패: {date_err}")
                        logger.info(f"[CRITICAL] 다른 형식으로 시도...")
                        data['Date'] = pd.to_datetime(data['Date'], errors='coerce')
                        logger.info(f"[CRITICAL] coerce 변환 결과: {data['Date'].head().tolist()}")
                    
                    data.set_index('Date', inplace=True)
                    logger.info(f"[CRITICAL] 최종 인덱스 타입: {type(data.index)}")
                    logger.info(f"[CRITICAL] 최종 인덱스 샘플: {data.index[:5].tolist()}")
                else:
                    logger.info(f"[CRITICAL] Date 컬럼이 없습니다! 사용 가능한 컬럼: {list(data.columns)}")
            else:
                # 표준 컬럼이 없으면 원본 데이터 그대로 사용
                logger.warning("⚠️ 표준 컬럼이 없습니다, 원본 데이터 사용")
                data = raw_data.copy()
                
                # Date 인덱스 처리 (이미 인덱스인 경우가 많음)
            ...
            # 데이터 타입 변환 (존재하는 컬럼만)
            potential_numeric_columns = ['Open', 'High', 'Low', 'Close', 'Volume', 'open_pric', 'high_pric', 'low_pric', 'cur_prc', 'trde_qty']
            ...
            elif 'open_pric' in data.columns:
                ohlc_columns = ['open_pric', 'high_pric', 'low_pric', 'cur_prc']
                volume_column = 'trde_qty'
            
            if ohlc_columns:
                # 존재하는 컬럼만 처리
                existing_ohlc = [col for col in ohlc_columns if col in data.columns]
                if existing_ohlc:
                    data[existing_ohlc] = data[existing_ohlc].fillna(method='ffill').round(0)
            
            if volume_column and volume_column in data.columns:
                data[volume_column] = data[volume_column].fillna(0).astype(np.int64)
                
                # 볼륨 데이터 처리 - 0값 또는 이상치 처리  
                median_volume = data[data[volume_column] > 0][volume_column].median()
                zero_volume_count = (data[volume_column] == 0).sum()
                if zero_volume_count > 0:
                    data[volume_column] = data[volume_column].replace(0, median_volume if not pd.isna(median_volume) else 0)
            ...
            logger.info(f"키움증권 API 데이터 다운로드 성공: {ticker} {interval} {len(data)}개 캔들")
            
            # yfinance 호환성을 위한 날짜 오름차순 정렬
            if not data.index.is_monotonic_increasing:
                data = data.sort_index()
                logger.info(f"데이터를 오름차순으로 정렬했습니다: {data.index[0]} ~ {data.index[-1]}")
            ...
            if "500" in error_msg or "INTERNAL_SERVER_ERROR" in error_msg:
                logger.warning(f"키움증권 서버 500 에러 발생 (시도 {attempt + 1}/{retries}): {ticker}")
                logger.warning("서버 부하로 인한 일시적 오류로 판단됩니다.")
                
                # 500 에러 시 더 긴 대기 시간
                wait_time = delay * (attempt + 1)  # 점진적 증가
                if attempt < retries - 1:
                    logger.info(f"⏳ {wait_time}초 후 재시도... (500 에러 대응)")
                    time.sleep(wait_time)
                    continue
            else:
                logger.error(f"데이터 다운로드 실패 (시도 {attempt + 1}/{retries}): {ticker}")
                logger.error(f"에러 타입: {error_type}, 메시지: {error_msg}")
            ...
            # 최종 실패
            if attempt == retries - 1:
                logger.error(f"❌ {ticker} 데이터 다운로드 최종 실패 ({retries}회 시도)")
                logger.error("💡 해결책: 1) 잠시 후 재시도 2) 키움증권 서버 상태 확인 3) 요청 크기 축소")
                return None
