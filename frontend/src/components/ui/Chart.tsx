...
const Chart: React.FC<ChartProps> = ({
  ...
  const [originalYRange, setOriginalYRange] = useState<[number, number] | null>(null);
  const [isOnRightAxis, setIsOnRightAxis] = useState(false);
  const plotRef = useRef<any>(null);

  // plotly graph div 레퍼런스 및 리스너 상태/핸들러 저장용 refs
  const graphDivRef = useRef<any>(null);
  const listenersAttachedRef = useRef(false);
  const handlersRef = useRef<any>(null);

  // state 값을 핸들러에서 신선하게 읽기 위해 refs로도 동기화
  const isDraggingYRef = useRef(isDraggingY);
  const dragStartYRef = useRef(dragStartY);
  const originalYRangeRef = useRef(originalYRange);

  useEffect(() => { isDraggingYRef.current = isDraggingY; }, [isDraggingY]);
  useEffect(() => { dragStartYRef.current = dragStartY; }, [dragStartY]);
  useEffect(() => { originalYRangeRef.current = originalYRange; }, [originalYRange]);

  // Plot이 실제 DOM에 그려진 시점에 안전하게 리스너를 붙이기 위한 헬퍼
  const attachListenersToPlot = (plotElement: HTMLElement | null) => {
    if (!plotElement) return;
    if (listenersAttachedRef.current) return;
    console.log('attach listeners (via attachListenersToPlot)', !!plotElement);

    const handleMouseDown = (e: MouseEvent) => {
      const rect = plotElement.getBoundingClientRect();
      const relativeXFromRight = rect.right - e.clientX;
      if (relativeXFromRight < 80) {
        e.preventDefault();
        e.stopPropagation();
        setIsDraggingY(true);
        setDragStartY(e.clientY);

        const currentLayout = (plotElement as any)._fullLayout || (plotElement as any).layout;
        if (currentLayout && currentLayout.yaxis) {
          const range = currentLayout.yaxis.range;
          if (Array.isArray(range) && range.length === 2) {
            setOriginalYRange([range[0], range[1]]);
          }
        }
      }
    };
    ...
    const handleMouseMove = (e: MouseEvent) => {
      if (!isDraggingYRef.current || dragStartYRef.current === null || !originalYRangeRef.current) return;
      e.preventDefault();
      e.stopPropagation();
      const deltaY = e.clientY - dragStartYRef.current;
      const sensitivity = 0.5;
      const zoomFactor = Math.pow(1.01, deltaY * sensitivity);
      const yCenter = (originalYRangeRef.current[0] + originalYRangeRef.current[1]) / 2;
      const yHalfRange = (originalYRangeRef.current[1] - originalYRangeRef.current[0]) / 2;
      const newYRange: [number, number] = [
        yCenter - yHalfRange * zoomFactor,
        yCenter + yHalfRange * zoomFactor
      ];

      if ((plotElement as any)._fullLayout) {
        (plotElement as any)._fullLayout.yaxis.range = newYRange;
        if (window.Plotly) {
          window.Plotly.relayout(plotElement, { 'yaxis.range': newYRange });
        }
      }
    };
    ...
    const handleMouseMoveHover = (e: MouseEvent) => {
      try {
        const rect = plotElement.getBoundingClientRect();
        const relativeXFromRight = rect.right - e.clientX;
        const onRight = relativeXFromRight < 80;
        setIsOnRightAxis(onRight);
      } catch (err) {
        // ignore
      }
    };

    const handleMouseLeave = () => setIsOnRightAxis(false);

    const handleMouseUp = () => {
      setIsDraggingY(false);
      setDragStartY(null);
      setOriginalYRange(null);
    };

    plotElement.addEventListener('mousedown', handleMouseDown);
    document.addEventListener('mousemove', handleMouseMove);
    plotElement.addEventListener('mousemove', handleMouseMoveHover);
    plotElement.addEventListener('mouseleave', handleMouseLeave);
    document.addEventListener('mouseup', handleMouseUp);

    handlersRef.current = { handleMouseDown, handleMouseMove, handleMouseMoveHover, handleMouseLeave, handleMouseUp };
    listenersAttachedRef.current = true;
  };

  const detachListenersFromPlot = (plotElement: HTMLElement | null) => {
    if (!plotElement) return;
    const h = handlersRef.current;
    if (!h) return;
    plotElement.removeEventListener('mousedown', h.handleMouseDown);
    document.removeEventListener('mousemove', h.handleMouseMove);
    plotElement.removeEventListener('mousemove', h.handleMouseMoveHover);
    plotElement.removeEventListener('mouseleave', h.handleMouseLeave);
    document.removeEventListener('mouseup', h.handleMouseUp);
  ...
  useEffect(() => {
    return () => {
      if (graphDivRef.current) {
        detachListenersFromPlot(graphDivRef.current);
      }
    };
  }, []);

  // Plot callback에서 graphDiv가 준비되었을 때 호출할 함수들
  const handlePlotInitialized = (_figure: any, graphDiv: any) => {
    graphDivRef.current = graphDiv;
    attachListenersToPlot(graphDiv);
  };

  const handlePlotUpdate = (_figure: any, graphDiv: any) => {
    graphDivRef.current = graphDiv;
    if (!listenersAttachedRef.current) attachListenersToPlot(graphDiv);
  };
  ...
  // Y축 영역에서 마우스 다운 이벤트 처리
  useEffect(() => {
    const plotElement = document.querySelector('.js-plotly-plot') as HTMLElement;
    console.log('attach listeners', !!plotElement);
    if (!plotElement) return;

    const handleMouseDown = (e: MouseEvent) => {
      // Y축 영역 확인 (오른쪽 가격 라벨 영역)
      const rect = plotElement.getBoundingClientRect();
      const relativeX = e.clientX - rect.left;
      const relativeXFromRight = rect.right - e.clientX; // 오른쪽으로부터의 거리

      // Y축 오른쪽 라벨 영역인지 확인 (오른쪽 80px 정도)
      if (relativeXFromRight < 80) {
        e.preventDefault();
        e.stopPropagation();
        setIsDraggingY(true);
        ...
        // 현재 Y축 범위 저장
        const currentLayout = (plotElement as any).layout;
        // 오른쪽에 가격 라벨이 있더라도 yaxis.range 키를 사용하여 범위를 가져온다
        if (currentLayout && currentLayout.yaxis) {
          setOriginalYRange([currentLayout.yaxis.range[0], currentLayout.yaxis.range[1]]);
        }
      }
    };

    const handleMouseMove = (e: MouseEvent) => {
      if (!isDraggingY || dragStartY === null || !originalYRange) return;

      e.preventDefault();
      e.stopPropagation();
      const deltaY = e.clientY - dragStartY;
      
      // 드래그 거리에 따른 줌 팩터 계산 (위로 드래그 = 확대, 아래로 = 축소)
      const sensitivity = 0.5; // 감도 조절 (0.5는 더 부드러운 줌)
      // 드래그 방향에 따라 위로 드래그하면 확대(zoomFactor < 1), 아래로 드래그하면 축소(zoomFactor > 1)
      const zoomFactor = Math.pow(1.01, deltaY * sensitivity);
      
      // Y축 범위 중심점 계산
      const yCenter = (originalYRange[0] + originalYRange[1]) / 2;
      const yHalfRange = (originalYRange[1] - originalYRange[0]) / 2;
      ...
      // Plotly 레이아웃 업데이트
      // react-plotly.js에서는 직접 DOM 조작 대신 다른 방법 사용
      const plotlyDiv = plotElement as any;
      if (plotlyDiv._fullLayout) {
        plotlyDiv._fullLayout.yaxis.range = newYRange;
        // 강제로 리드로우
        if (window.Plotly) {
    ...
    // 호버 시 커서 변경을 위한 마우스 이동 핸들러 (오른쪽 축 영역 감지)
    const handleMouseMoveHover = (e: MouseEvent) => {
      try {
        const rect = plotElement.getBoundingClientRect();
        const relativeXFromRight = rect.right - e.clientX;
    ...
    plotElement.addEventListener('mousedown', handleMouseDown);
    document.addEventListener('mousemove', handleMouseMove);
    // 플롯 내부 이동 리스너는 hover용으로 plotElement에 등록
    plotElement.addEventListener('mousemove', handleMouseMoveHover);
    plotElement.addEventListener('mouseleave', handleMouseLeave);
    document.addEventListener('mouseup', handleMouseUp);

    return () => {
      plotElement.removeEventListener('mousedown', handleMouseDown);
      document.removeEventListener('mousemove', handleMouseMove);
      plotElement.removeEventListener('mousemove', handleMouseMoveHover);
      plotElement.removeEventListener('mouseleave', handleMouseLeave);
      document.removeEventListener('mouseup', handleMouseUp);
    };
  }, [isDraggingY, dragStartY, originalYRange]);
    ...
    <div className="w-full h-full" ref={plotRef}>
      <Plot
        ...
        style={{ 
          width: '100%', 
          height: '100%',
          cursor: isDraggingY || isOnRightAxis ? 'ns-resize' : 'auto'
        }}
